...
export function getTileZRotation() {
  return RandomInNegativeRange(0.7);
}
...
export default function Dump(props) {
  ...
  const filmCount = useMemo(
    () => films.filter((film) => !film.filler).length,
    []
  );
  ...
  const logline = filmIsSelected ? (
    <div className={"logline"} onClick={() => setSelected()}>
      {[films[selectedIndex].logline]}
    </div>
  ) : undefined;
