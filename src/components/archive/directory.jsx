...
const getShouldHighlight = (isSelected, isHovered, film) =>
  isSelected || (!film.filler && isHovered);
...
function ListElement({ film, isSelected, setSelected, setShowFilm }) {
  ...
  const listInfo = filler ? (
    ...
    <div className={"listInfo"}>
      <span className={getHoveredClassName("listTitle", shouldHighlight)}>
        {title.toUpperCase()}
      </span>
      <span className={getHoveredClassName("listFilmmaker", shouldHighlight)}>
        {getFilmmakerNames(film).toLowerCase()}
      </span>
    </div>
  );
  ...
  return (
    <div className={"listRow"}>
      <div
        id={`film${index}`}
        className={getHoveredClassName(
          `listRowInfo ${orientation} ${isFiller}`,
          shouldHighlight
        )}
        ...
        style={
          filler
            ...
            : { cursor: "pointer", ...fadeInDelay, ...animationDuration }
        }
      >
        {listInfo}
        {
...
function YearButton({ year, setYear, currentYear }) {
  ...
  const finalClassName =
    isHovered || currentYear === year ? className + " hovered" : className;
  return (
    <div
